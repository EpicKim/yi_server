...
class ProductController extends Controller
{
    ...
    protected function grid()
    {
        return Admin::grid(ProductModel::class, function (Grid $grid) {
            ...
            $grid->color('颜色')->display(function ($color) {
                if (strlen($color) >= 8) {
                    return mb_substr($color, 0, 8) . '...';
                }
                return $color;
            });
            $grid->size('尺寸')->display(function($size){
               if(strlen($size)>=8){
                   return mb_substr($size,0,8).'...';
               }
               return $size;
            });
    ...
    protected function form()
    {
        return Admin::form(ProductModel::class, function (Form $form) {
            ...
            $form->text('size', '尺寸(以||分割)')->default('S||M||L||XL');
            $form->text('color', '颜色(以||分割)');
    ...
    public function store(Request $request)
    {
        ...
        if ($size == '' || count(explode($size, '||')) == 0) {
            $size = '';
        }
        if ($color == '' || count(explode($color, '||')) == 0) {
            $color = '';
        }
